...
music.tr$genre <- as.numeric(as.integer(factor(music.tr$genre)))
knn.cls<-knn(train = music.tr[, 1:13], test = music.tst[, 1:13], music.tr[, 14], k=16)
...
set.seed(2)
## Bagging 
bag <- randomForest(as.factor(genre)~., data = music.tr, mtry=14, importance = TRUE, proximity = TRUE)
...
s1 <- svm(genre~., data=music.tr, type='C', scale=TRUE)
